...
pub fn t() -> String {
    let mut connection = ft_sdk::default_pg().expect("failed to connect to the database");
...
fn other_insertable(c: &mut ft_sdk::PgConnection) {
...
pub fn chrono(c: &mut ft_sdk::PgConnection) {
    let user = User4 {
        id: 1,
        username: "yo".to_string(),
...
pub fn chrono_datetime(c: &mut ft_sdk::PgConnection) {
...
pub fn chrono_datetime_utc(c: &mut ft_sdk::PgConnection) {
    ...
    let c = diesel::insert_into(crate::diesel::ft_user_6::table)
        .values(user)
        .returning(crate::diesel::ft_user_6::id)
        .get_result::<i64>(c)
        .unwrap();
}
